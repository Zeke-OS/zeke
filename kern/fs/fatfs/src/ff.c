...
FRESULT dir_alloc (
    FF_DIR * dp,    /* Pointer to the directory object */
    unsigned int nent   /* Number of contiguous entries to allocate (1-21) */
)
{
    FRESULT res;
    unsigned int n;
...
static
FRESULT dir_find (
    FF_DIR * dp         /* Pointer to the directory object linked to the file name */
)
{
    FRESULT res;
    uint8_t c, *dir;
 ...
 */
static FRESULT find_volume (FATFS ** rfs, const TCHAR ** path, uint8_t wmode)
{
    uint8_t fmt;
    int vol;
    DSTATUS stat;
...
)
{
    ...
    if (_MULTI_PARTITION && part) {
        ...
        tbl = &fs->win[MBR_Table + (part - 1) * SZ_PTE];
        tbl[4] = sys;
        if (fatfs_disk_write(pdrv, fs->win, 0, SS(fp->fs)))    /* Write it to teh MBR */
            return FR_DISK_ERR;
        md = 0xF8;
    } else {
        if (sfd) {  /* No partition table (SFD) */
            ...
            if (fatfs_disk_write(pdrv, fs->win, 0, SS(fp->fs)))    /* Write it to the MBR */
                return FR_DISK_ERR;
            md = 0xF8;
        }
    }
    ...
    if (fatfs_disk_write(pdrv, tbl, b_vol, SS(fp->fs)))    /* Write it to the VBR sector */
        return FR_DISK_ERR;
    if (fmt == FS_FAT32)                    /* Write backup VBR if needed (VBR+6) */
        fatfs_disk_write(pdrv, tbl, b_vol + 6, SS(fp->fs));
    ...
    for (i = 0; i < N_FATS; i++) {      /* Initialize each FAT copy */
        ...
        if (fatfs_disk_write(pdrv, tbl, wsect++, SS(fp->fs)))
            return FR_DISK_ERR;
        memset(tbl, 0, SS(fs));            /* Fill following FAT entries with zero */
        for (n = 1; n < n_fat; n++) {
            /*
             * This loop may take a time on FAT32 volume due to many single
             * sector writes
             */
            if (fatfs_disk_write(pdrv, tbl, wsect++, SS(fp->fs)))
                return FR_DISK_ERR;
    ...
    if (fmt == FS_FAT32) {
        ...
        fatfs_disk_write(pdrv, tbl, b_vol + 1, SS(fp->fs));    /* Write original (VBR+1) */
...
{
    unsigned int i, n, sz_cyl, tot_cyl, b_cyl, e_cyl, p_cyl;
