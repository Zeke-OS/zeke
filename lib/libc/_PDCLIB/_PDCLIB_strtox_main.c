/*
 * _PDCLIB_strtox_main(const char * *, int, _PDCLIB_uintmax_t,
 *                     _PDCLIB_uintmax_t, int)
 *
...
_PDCLIB_uintmax_t _PDCLIB_strtox_main(const char ** p, unsigned int base,
        uintmax_t error, uintmax_t limval, int limdigit, char * sign)
{
    ...
    while ((x = memchr(_PDCLIB_digits, tolower(**p), base)) != NULL) {
        ...
        if ((rc < limval) || ((rc == limval) && (digit <= limdigit))) {
            ...
            errno = ERANGE;
            /*
             * TODO: Only if endptr != NULL - but do we really want *another*
             * parameter?
             */
            /*
             * TODO: Earlier version was missing tolower() here but was not
             * caught by tests
             */
            while (memchr(_PDCLIB_digits, tolower(**p), base) != NULL) ++(*p);
            /*
             * TODO: This is ugly, but keeps caller from negating
             * the error value
             */
            *sign = '+';
            return error;
